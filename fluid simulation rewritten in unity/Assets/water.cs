...
public class Water : MonoBehaviour{
    ...
    public float nearPressureMultiplier = .01f;
    public ComputeShader computeShader;
    public Material material;

    // creates a 3d cube of particles with even spacing between particles
    void createWaterCube(ref List<float3> particles, ref List<float3> velocities, float3 offset, float size, int rows, int cols, int planes){
        ...
        for (int i = 0; i < rows; ++i){
            ...
            for (int j = 0; j < cols; ++j){
                ...
                for (int k = 0; k < planes; ++k){
                    ...
                    particles.Add(pos + new float3(rx, ry, rz));
                    velocities.Add(new float3(0, 0, 0));
                }
            }
        }
    }
    ...
    void Start(){
        ...
        List<float3> particles = new List<float3>();
        List<float3> velocities = new List<float3>();
        ...
        particleBuffer = new ComputeBuffer(particleCount, sizeof(float) * 3);
        particleBuffer.SetData(particles);
        velocitiesBuffer = new ComputeBuffer(particleCount, sizeof(float) * 3);
        velocitiesBuffer.SetData(velocities);
        predictedBuffer = new ComputeBuffer(particleCount, sizeof(float) * 3);
